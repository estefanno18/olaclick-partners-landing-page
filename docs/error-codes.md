...
> **API Reference:** [Errors](https://developers.olaclick.app/docs/api) — Error format in the Public API

## Authentication Errors

| HTTP Code | Error Code | Description |
|-----------|-----------|-------------|
| 401 | `invalid_client` | The credentials (`client_id`/`client_secret`) are not valid, or the `company_id` does not have a connection with your connector |
| 401 | `TOKEN_EXPIRED` | The access token has expired. Request a new one via [`POST /v1/oauth/token`](https://developers.olaclick.app/docs/api) |
...
| 404 | `CONNECTION_NOT_FOUND` | The connection with the provided ID does not exist |
...
| 409 | `CONNECTION_ALREADY_ACTIVE` | The connection is already active |
